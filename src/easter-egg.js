    ...
    iphone: {
      ...
      load: function (link) {
        ...
        easter.addEvent(document, 'touchmove', function (e) {
          if (e.touches.length == 1 && easter.iphone.capture == true) {
            var touch = e.touches[0];
            easter.iphone.stop_x = touch.pageX;
            easter.iphone.stop_y = touch.pageY;
            easter.iphone.tap = false;
            easter.iphone.capture = false;
            easter.iphone.check_direction();
          }
        });

        easter.addEvent(document, 'touchend', function (evt) {
          if (easter.iphone.tap == true) {
            easter.iphone.check_direction(link);
          }
        }, false);

        easter.addEvent(document, 'touchstart', function (evt) {
          easter.iphone.start_x = evt.changedTouches[0].pageX;
          easter.iphone.start_y = evt.changedTouches[0].pageY;
          easter.iphone.tap = true;
          easter.iphone.capture = true;
        });
      },
